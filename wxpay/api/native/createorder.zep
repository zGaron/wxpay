...
class CreateOrder extends Api
{
    ...
    public api_url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
    ...
    public function useMode(int nativeMode = 1) -> <\WXPay\Api\Native\CreateOrder>
    {
        if nativeMode == 1 {

            return this;
        }
        let this->native_mode = nativeMode;
    ...
    {
        array fields = ["appid", "mch_id", "time_stamp", "nonce_str", "product_id", "sign"];
        string qrcode = "weixin://wxpay/bizpayurl?";
        var field;
        ...
        for field in fields {
            if field == "time_stamp" {
                let qrcode .= field . "=" . time() . "&";
                continue;
            }
            let qrcode .= field . "=" . this->{field} . "&";
        }
        return qrcode->trimright("&");
    }
    ...
    public function fire()
    {
        ...
        if this->native_mode == CreateOrder::MODE_NATIVE_1 {
            let shortUrl = this->generateLink();
            let parameter["long_url"] = shortUrl;
            let vars = Shorturl::make(parameter)->signWith(this->_config);
            return vars->fire();
        } else {
            return this->_fire();
        }
